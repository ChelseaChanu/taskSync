...
function TaskCard() {
  ...
  const [showModal, setShowModal] = useState(false); // For assignee submission
  const [viewModal, setViewModal] = useState(false); // For assigner to view submissions
  ...
  // Upload submission for assignee
  ...
  // Fetch all submissions for assigner
  ...
  // Check if current user is assigner
          ...
          <div className="bg-gray-100 rounded-lg p-6 w-[90%] max-w-lg max-h-[80vh] overflow-y-auto">
            ...
            {submissions.length === 0 ? (
              ...
              submissions.map((sub, idx) => {
                const userObj = task.assignedToObjects.find(
                  u => u.uid === sub.submittedBy
                );
                const name = userObj
